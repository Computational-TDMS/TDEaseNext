...
- Backend registry/schema smoke (`uv run pytest tests/test_interactive_execution_simple.py -v`)
  - PASS: TopMSV tool definitions load and state-port contracts validate.
- Backend service tests attempted (`uv run pytest tests/unit/services/test_prsm_bundle_builder.py tests/unit/services/test_node_data_service.py -v`)
  - BLOCKED by sandbox filesystem permissions on `tmp_path` temp dirs (`PermissionError: [WinError 5] Access denied`).
- Backend functional smoke checks executed via `uv run python -` script
  - PASS: `build_prsm_bundle` outputs contract fields, TopN behavior, optional feature enrichment.
  - PASS: `parse_tabular_file` handles TopPIC preamble and raises on invalid TSV header detection.
- Workflow wiring validation executed via `uv run python -` script over `workflows/wf_test_full.json`
  - PASS: required TopMSV nodes/edges exist, selection edges are `connectionKind: "state"`, `skip_html_folder=true` is set for `topfd_1` and `toppic_1`.
- Frontend automated tests attempted (`pnpm --dir TDEase-FrontEnd test:run`)
  - BLOCKED by sandbox process-spawn restriction (`Error: spawn EPERM` from Vite/esbuild startup).
- Frontend type-check fallback (`TDEase-FrontEnd\\node_modules\\.bin\\vue-tsc --noEmit --project TDEase-FrontEnd\\tsconfig.json`)
  - FAIL (pre-existing unrelated issue): `src/components/visualization/SpectrumViewer.vue(275,32) TS18049: 's' is possibly 'null' or 'undefined'.`
