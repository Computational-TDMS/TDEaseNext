#!/usr/bin/env python3
"""
Mock Tool C - 模拟质谱工作流第三步
读取 B 的输出 (*_b.txt)，输出 {sample}_c.txt，产生模拟日志。
必须验证输入文件包含 TOOL_B_SIGNATURE 才继续，否则退出码非 0。
"""
import argparse
import sys
...
# 必须从 B 的输出中读取到的签名，用于验证真正读到文件
EXPECTED_SIGNATURE_KEY = "TOOL_B_SIGNATURE"


def main():
    parser = argparse.ArgumentParser(description="Tool C: 处理 Tool B 的输出")
    parser.add_argument("-i", "--input", action="append", required=True, help="输入文件 (来自 Tool B)")
    parser.add_argument("-o", "--output", required=True, help="输出文件路径，如 {sample}_c.txt")
    ...
    log_lines.append(f"[{datetime.now().isoformat()}] Input validation OK")
    log_lines.append(f"[{datetime.now().isoformat()}] Tool C completed successfully")
