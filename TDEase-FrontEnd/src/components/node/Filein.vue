...
<template>
  <li class="io-item" :class="{ 'has-files': row.samples && row.samples.length > 0 }">
    <div class="io-content">
      <span class="io-name" :title="`${row.port.name || row.port.id} (${row.port.type || 'file'})`">
        {{ row.port.name || row.port.id }}
        <em v-if="row.port.required && !row.connected" style="color:#e6a23c; margin-left:6px; font-style: normal;">缺少输入文件</em>
      </span>
      
      <div v-if="row.samples && row.samples.length" class="file-list">
        <div 
          v-for="(s, idx) in row.samples" 
          :key="`${row.port.id}-${idx}-${s}`" 
          class="file-pill"
    ...
    <Handle 
      v-if="!(row.file && row.samples && row.samples.length)"
      type="target" 
      :position="Position.Left" 
      :id="`input-${row.port.id}`"
